...
sleep <- na.omit(sleep[, c("Sleep.duration","Sleep.efficiency", "Age", "Caffeine.consumption", "Alcohol.consumption", "Smoking.status")])
...
```
Now, we have a significant intercept, and the p value for alcohol consumption is lower. This does not give us many variables to work with, so it looks like we should go back and explore more, maybe with interaction effects. What we can do is run the logistic, but accounting for all interaction terms.
